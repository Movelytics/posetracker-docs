...
| **Packed tarball** | ~**10.0 MB** | ~**212 kB** (~47× smaller) |
| **Unpacked** | ~14.8 MB | ~805 kB |
...
`model: 'blazepose'` loads BlazePose (lite) via CDN
`@tensorflow-models/pose-detection` in the WebView. Keypoints stay COCO-17.
Heavier than MoveNet — prefer MoveNet on mid-range devices. Offline package
does not support BlazePose.
...
| Demo | [Demo](https://github.com/Movelytics/react-native-pose-estimation-demo) · [Snack](https://snack.expo.dev/@fsepret/posetracker-sdk-demo-app) | [Demo](https://github.com/Movelytics/react-native-pose-estimation-light-demo) · [Snack](https://snack.expo.dev/@fsepret/posetracker-sdk-light-demo-app) |

## Next

Continue with [Try on your phone](/try-expo-go), [Quickstart](/quickstart), [Permissions](/permissions), or [Provider options](/reference/provider-client) (including light-only `model` / `modelUrl`).
