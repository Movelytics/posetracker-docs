...
```tsx
<WebViewPoseView source="image" sourceUri={uri} style={{ flex: 1 }} />
<WebViewPoseView source="video" sourceUri={uri} style={{ flex: 1 }} />
```

Web: `pt.setSource({ type: 'image' | 'video', src })` then `start()` / `analyze()`.

Full guide (CORS, `content://`, React, script tag): [Media sources](/media-sources).
